...
<!-------------------------------------- MAIN -->
	...
	<table cellpadding="0" cellspacing="0" class="bar">
		<tr><td width="200"><a href="insert_city.php" class="add" target="_self" class="" >新增縣市</a></td></tr>
	</table>
	...
	<tr>
      ...
      <tbody>
         <?php
		$sql_city_array = "SELECT *  FROM  city_array ORDER BY city_sort ASC ";
		$result_city_array = execute_sql($database_name, $sql_city_array, $link);
		while ($row_city_array = mysql_fetch_assoc($result_city_array)    )
		{
		?>
		<tr>
		<td> 
		<input type="number" name="sort[]"  value="<?=$row_city_array['city_sort'];?>" class="num">
		<input type="hidden" name="sort_id[]"  value="<?=$row_city_array['id'];?>" >
		</td>
		<td><?=$row_city_array['city_name'];?></td>
		<td><a href="fix_city.php?id=<?=$row_city_array['id'];?>"><img src="../images/icon_edit.png"  class="adm_icon"align="absmiddle"></a></td>
		<td>
		<a href="javascript:if(confirm('確定要刪除嗎??'))location='mng_city_proc.php?mode=dell_city&id=<?=$row_city_array['id'];?>'"><img src="../images/icon_del.png"  class="adm_icon"align="absmiddle"></a>
		</td>
		</tr>	
		<?php
		}
