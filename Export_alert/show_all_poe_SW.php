<!--------dataTablesw套件---------->
		...
		if($_GET['mode']=='query')
		{
			...
			<tbody>
			<?php
			$sql_router = "SELECT *  FROM  ass_poesw  where ass_poesw_label='$label_row'   ";
			$result_router = execute_sql($database_name, $sql_router, $link);
			while ($row_router = mysql_fetch_assoc($result_router))
			{
					$ass_ap_city =  $row_router['ass_poesw_city'];
					$ass_ap_twon =  $row_router['ass_poesw_twon'];
					$ass_ap_tribe =  $row_router['ass_poesw_tribe'];
					$ass_ap_ip =  $row_router['ass_poesw_ip'];
					$ass_ap_name =  $row_router['ass_poesw_name'];
    ...
    $(document).ready(function(){ 
      var opt={ "oLanguage":{"sProcessing":"處理中...",
				...
				"iDisplayLength": 80,
				 dom: 'Bfrtip',	 buttons: 
				 [
					{ extend: 'excelHtml5', text: '匯出斷線歷史紀錄' ,title: '<?= date("Y-m-d");?>PoE S/W斷線歷史紀錄' },
      ...
      $("#show_date").dataTable(opt);
		...
		?>
			...
			<tbody>
			<?php
			$sql_router = "SELECT *  FROM  ass_poesw  where ass_poesw_label =2  ";
			$result_router = execute_sql($database_name, $sql_router, $link);
			while ($row_router = mysql_fetch_assoc($result_router))
			{
					$ass_ap_city =  $row_router['ass_poesw_city'];
      ...
      var opt={ "oLanguage":{"sProcessing":"處理中...",
				...
				lengthMenu: [
				 ...
				 [
					...
					{ extend: 'print', text: '列印',title: '<?= date("Y-m-d");?>POE_SW斷線數量統計表' },	
					'pageLength',						
				],
	       };
      $("#show_date").dataTable(opt);
