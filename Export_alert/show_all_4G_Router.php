<!--------dataTablesw套件---------->
		...
		if($_GET['mode']=='query')
		{
			...
			<tbody>
			<?php
			$sql_router = "SELECT *  FROM  ass_4Ggrouter  where ass_4Ggrouter_label  ='$label_row'  ";
			$result_router = execute_sql($database_name, $sql_router, $link);
			while ($row_router = mysql_fetch_assoc($result_router))
			{
					$ass_ap_city =  $row_router['ass_4Ggrouter_city'];
					$ass_ap_twon =  $row_router['ass_4Ggrouter_twon'];
					$ass_ap_tribe =  $row_router['ass_4Ggrouter_tribe'];
					$ass_ap_ip =  $row_router['ass_4Gip'];
					$ass_ap_name =  $row_router['ass_4Gname'];
    ...
    $(document).ready(function(){ 
      var opt={ "oLanguage":{"sProcessing":"處理中...",
				 ...
				 [
					...
					{ extend: 'print', text: '列印',title: '<?= date("Y-m-d");?>4G Router斷線數量統計表' },		
					'pageLength',				
				],
	       };
      $("#show_date").dataTable(opt);
		...
		?>
			...
			<tbody>
			<?php
			$sql_router = "SELECT *  FROM  ass_4Ggrouter  where ass_4Ggrouter_label =2  ";
			$result_router = execute_sql($database_name, $sql_router, $link);
			while ($row_router = mysql_fetch_assoc($result_router))
			{
					$ass_ap_city =  $row_router['ass_4Ggrouter_city'];
