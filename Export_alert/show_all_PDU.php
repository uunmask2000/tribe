<!--------dataTablesw套件---------->
		...
		if($_GET['mode']=='query')
		{
			...
			<tbody>
			<?php
			$sql_router = "SELECT *  FROM  ass_pdu  where ass_pdu_label ='$label_row'   ";
			$result_router = execute_sql($database_name, $sql_router, $link);
			while ($row_router = mysql_fetch_assoc($result_router))
			{
					$ass_ap_city =  $row_router['ass_pdu_city'];
					$ass_ap_twon =  $row_router['ass_pdu_twon'];
					$ass_ap_tribe =  $row_router['ass_pdu_tribe'];
					$ass_ap_ip =  $row_router['ass_pdu_ip'];
					$ass_ap_name =  $row_router['ass_pdu_name'];
    ...
    $(document).ready(function(){ 
      ...
      $("#show_date").dataTable(opt);
		...
		?>
			...
			<tbody>
			<?php
			$sql_router = "SELECT *  FROM  ass_pdu  where ass_pdu_label =2  ";
			$result_router = execute_sql($database_name, $sql_router, $link);
			while ($row_router = mysql_fetch_assoc($result_router))
			{
					$ass_ap_city =  $row_router['ass_pdu_city'];
    ...
    $(document).ready(function(){ 
      var opt={ "oLanguage":{"sProcessing":"處理中...",
				...
				lengthMenu: [
				 ...
				 [
					{ extend: 'excelHtml5', text: '匯出斷線歷史紀錄' ,title: '<?= date("Y-m-d");?>PDU斷線歷史紀錄' },
					{ extend: 'print', text: '列印',title: '<?= date("Y-m-d");?>PDU斷線數量統計表' },	
					'pageLength',						
				],
	       };
      $("#show_date").dataTable(opt);
