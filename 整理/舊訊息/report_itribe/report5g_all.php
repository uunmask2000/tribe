...
<!-------------------------------------- MAIN -->
	<div id="main">
		...
		<div id="div_print"><style>table td { padding:5px;} table th { padding:5px; border:#000 1px solid;}</style>
			<table class="tablesorter" id="table1">
				<thead>
				<tr>
				<th colspan="6">iTribe 5G 服務效益總表</th> 
				</tr>
				<tr>
					<th>月份</th>
					<th>使用人數</th>
	...
	<?php
	 require_once("dbtools.inc.php");
		...
		for($x=1;$x<=12;$x++)
		{
			$sql= "SELECT SUM(`of_clients_5G`),SUM(`tx_5G`),SUM(`rx_5G`),SUM(`busy_5G`) FROM Client_Number_Vs_Air_Time_5G WHERE `timestamp_5G` LIKE '$moon[$x]/%' and `timestamp_5G` LIKE '%$today_year%' ";
