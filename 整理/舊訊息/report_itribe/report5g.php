	...
	<div id="main">
			...
			<form action="" method="post">
				<select name="zone_change" size="1" onchange="this.form.submit();"> 
					<option   value="all" <?php if($_POST['zone_change']=='all'){echo 'selected'; }?>>show_all</option>
						<?php
						$sql_zone = "SELECT zone_5G  FROM `Client_Number_Vs_Air_Time_5G` GROUP BY `zone_5G` ";
						$result_zone = execute_sql($database_name, $sql_zone, $link); 
						while($row_zone = mysql_fetch_assoc($result_zone))
						{
						?>
						<option value="<?=$row_zone['zone_5G'];?>" <?php if($_POST['zone_5G_change']==$row_zone['zone_5G']){echo 'selected'; }?> ><?=$row_zone['zone_5G'];?></option>		
						<?php
						}
						?>
				</select>
		...
		<div id="div_print"><style>table td { padding:5px;} table th { padding:5px; border:#000 1px solid;}</style>
			<table class="tablesorter" id="table1">
				<thead>
					<tr>
					<th colspan="8">iTribe 5G 各設備服務效益總表</th> 
					</tr>
					<tr>
						<th>No.</th>
						<th>熱點名稱</th>
				...
				<tbody>
				<?php
				$zone_5G_change = $_POST['zone_5G_change'];
				//echo $zone_5G_change;
				if($zone_5G_change==NULL)
				{
					$sql0 = "SELECT zone_5G  FROM `Client_Number_Vs_Air_Time_5G` GROUP BY `zone_5G` ";	
				}else if($zone_5G_change=='all')
				{
					$sql0 = "SELECT zone_5G  FROM `Client_Number_Vs_Air_Time_5G` GROUP BY `zone_5G` ";	
				}
				else
				{
					$sql0 = "SELECT zone_5G  FROM `Client_Number_Vs_Air_Time_5G` where zone_5G ='$zone_5G_change' GROUP BY zone_5G ";
				}
				//echo $sql0;
				...
				$sql= "SELECT zone_5G,SUM(`of_clients_5G`),SUM(`tx_5G`),SUM(`rx_5G`),SUM(`busy_5G`) FROM Client_Number_Vs_Air_Time_5G WHERE zone_5G='$zone'  ";		
				$result = execute_sql($database_name, $sql, $link);

				while ($row = mysql_fetch_assoc($result) )
							{
								$of_clients=$row['SUM(`of_clients_5G`)'];
								$busy= $row['SUM(`busy_5G`)'];
								$tx = ($row['SUM(`tx_5G`)']/1024);
								$rx = ($row['SUM(`rx_5G`)']/1024);
								$sum_rx_tx = $tx +  $rx ;
