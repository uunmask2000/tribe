...
<div id="wrap">
    ...
    <h1 class="table_name" align="center">TX_and_Rx</h1>
		...
		$sql = "SELECT  ssid_itaiwan, SUM(itaiwan_rx),SUM(itaiwan_tx),ssid_itribe ,SUM(itribe_rx) ,SUM(itribe_tx)   FROM TX_and_Rx WHERE `timestamp` LIKE '$moon[$i]/%' and `timestamp` LIKE '%$today_year%'  ORDER BY timestamp DESC";	
...
<?php
	...
	//建立資料連接
	$link = create_connection();
	$sql = "SELECT  ssid_itaiwan_5G, SUM(itaiwan_rx_5G),SUM(itaiwan_tx_5G),ssid_itribe_5G,SUM(itribe_rx_5G) ,SUM(itribe_tx_5G)   FROM TX_and_Rx_5G WHERE `timestamp_5G` LIKE '$moon[$i]/%' and `timestamp_5G` LIKE '%$today_year%'  ORDER BY timestamp_5G DESC";	
	$result = execute_sql($database_name, $sql, $link);
		...
		echo $moon[$i].'月<br/>';
				...
				while ($row = mysql_fetch_assoc($result))
				{
					...
					echo 'SUM(itaiwan_rx_5G):'.$row['SUM(itaiwan_rx_5G)'];echo '<br>';
					echo 'SUM(itaiwan_tx_5G):'.$row['SUM(itaiwan_tx_5G)'];echo '<br>';

					echo $row['ssid_itribe_5G'];echo '<br>';
					echo 'SUM(itribe_rx_5G):'.$row['SUM(itribe_rx_5G)'];echo '<br>';
					echo 'SUM(itribe_tx_5G):'.$row['SUM(itribe_tx_5G)'];echo '<br>';
				}
		echo '</div>';
