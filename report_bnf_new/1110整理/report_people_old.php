...
$sql="SELECT SUM(acctsessiontime),SUM(acctinputoctets),SUM(acctoutputoctets),SUM(acctstartdelay),SUM(acctstopdelay),acctstarttime  FROM radacct  where nasipaddress IN ('$ass_ap_ip') and $key_sql and acctstarttime like '%$year-$month%' ";
...
///
$sql2="SELECT username   FROM radacct where  nasipaddress IN ('$ass_ap_ip')  and $key_sql and acctstarttime like '%$year-$month%'  GROUP BY username ";
...
$(document).ready(function(){ 
  var opt={ "oLanguage":{"sProcessing":"處理中...",
			 ...
			 [
				...
				{ extend: 'print', text: '列印',title: '<?= date("Y-m-d");?> <?=$filename;?>  使用人次統計分析明細表' },	
				'pageLength',				
			],
	   };
