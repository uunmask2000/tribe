	...
	<div id="main">
			...
			<form action="" method="post">
				<select name="nasipaddress" size="1" onchange="this.form.submit();"> 
					<option value="all" <?php if($_POST['nasipaddress']=='all'){echo 'selected'; }?>>列出所有</option>
						...
						$sql_zone = "SELECT *  FROM radacct where realm<>'itw'  GROUP BY nasipaddress ORDER BY nasipaddress desc";
		...
		<div id="div_print"><style>table td { padding:5px;} table th { padding:5px; border:#000 1px solid;}</style>
			<table class="tablesorter" id="table1">
				<thead>
					<tr>
					<th colspan="12" class="table_tt">iTirbe 各Ap(ip)   服務效益總表</th> 
					</tr>
					<tr>
						<th width="60">日期</th>
						<th>使用人次</th>
				...
				{
					$sql0 = "SELECT *  FROM radacct where realm<>'itw' and  realm='NULL'  or realm='' and acctstarttime like '%$like_text%'  GROUP BY nasipaddress";	
				...
				{
					$sql0 = "SELECT *  FROM radacct where nasipaddress='$nasipaddress' and realm<>'itw' and  realm='NULL'  or realm='' and acctstarttime like '%$like_text%' GROUP BY nasipaddress";
				}
				//echo $sql0;
					...
					<tr>
						...
						<?php
							...
							while($row_RUS = mysql_fetch_assoc($RUS))
								...
								//echo  '<td colspan="4">'.$nasipaddress.'</td>'; 
								}
