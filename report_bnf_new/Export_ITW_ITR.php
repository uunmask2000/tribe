					  ...
					  ?>
							...
							while ($row_ap= mysql_fetch_assoc($result_ap)  )
							{
								...
								//echo '<br>';
											...
											//$sql_sum="SELECT SUM(acctsessiontime),SUM(acctinputoctets),SUM(acctoutputoctets),SUM(acctstartdelay),SUM(acctstopdelay),acctstarttime  FROM radacct  where nasipaddress IN ('$ass_ap_ip')  and acctstarttime like '%$day%'   ";
											$sql_sum="SELECT SUM(acctsessiontime),SUM(acctinputoctets),SUM(acctoutputoctets),SUM(acctstartdelay),SUM(acctstopdelay),acctstarttime  FROM radacct  where nasipaddress IN ('$ass_ap_ip')  and acctstarttime like '%$day1%'    ";
											
											$result_sum = execute_sql($database_name, $sql_sum, $link);
								echo	$sql_sum;	
								echo '<br>';
											//$sql1="SELECT radacctid FROM radacct where  nasipaddress IN ('$ass_ap_ip')  and acctstarttime like '%$day%' ";
											$sql1="SELECT radacctid FROM radacct where  nasipaddress IN ('$ass_ap_ip')  and acctstarttime like '%$day1%'  ";
											$result1 = execute_sql($database_name, $sql1, $link);
											$number1 = mysql_num_rows($result1);
											///
											//$sql2="SELECT callingstationid   FROM radacct where  nasipaddress IN ('$ass_ap_ip')  and acctstarttime like '%$day%' GROUP BY callingstationid ";
											$sql2="SELECT callingstationid   FROM radacct where  nasipaddress IN ('$ass_ap_ip')  and acctstarttime like '%$day1%' GROUP BY callingstationid ";
...
$(document).ready(function(){ 
  var opt={ "oLanguage":{"sProcessing":"處理中...",
			 ...
			 dom: 'Bfrtip',	 buttons: 
			 [
					{ extend: 'excelHtml5', text: '匯出服務效益月報表' ,title: '<?= date("Y-m-d");?>服務效益月報表' },
