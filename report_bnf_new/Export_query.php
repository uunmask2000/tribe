...
<!-------------------------------------- MAIN -->
	<div id="main">
		...
		<div class="report_nav">
			<a href="/report_bnf_new/report_all.php">年報表</a>
			<a href="/report_bnf_new/Export_query.php" target="_blank" 
			<?php if (preg_match("/Export_query/i", $_SERVER['PHP_SELF'])) {echo 'style="background: #126389"'; } ?>
			>月報表</a>
		</div>
		...
		<select name="realm">
			...
			<option value="all" <?php if($_POST['realm']=='all'){echo 'selected'; }?> >全部</option>
...
if($realm=="all")
{
		...
		//echo '<br>';
		///
		$sql2="SELECT callingstationid   FROM radacct where  nasipaddress IN ('$ass_ap_ip') and realm<>'itw' and acctstarttime like '%$day%' GROUP BY callingstationid ";
	...
	$sql_tribe="SELECT * FROM `tribe` where tribe_label='$label_tribe'  ORDER BY `tribe_label`,`city_id`,`township_id`,`tribe_name` ASC ";
		...
		$ii ++;				
			...
			//echo '<br>';
			///
			$sql2="SELECT callingstationid   FROM radacct where  nasipaddress IN ('$ass_ap_ip') and $key and acctstarttime like '%$day%' GROUP BY callingstationid ";
			$result2 = execute_sql($database_name, $sql2, $link);
			$number2 = mysql_num_rows($result2);
...
$(document).ready(function(){ 
  var opt={ "oLanguage":{"sProcessing":"處理中...",
			 ...
			 dom: 'Bfrtip',	 buttons: 
			 [
				{ extend: 'excelHtml5', text: '匯出<?=$key_sting ;?>服務效益月報表' ,title: '<?= date("Y-m-d");?><?=$key_sting ;?><?=$day;?>月服務效益月報表' },
