...
<!-------------------------------------- MAIN -->
	<div id="main">
		<?php
		    if($_GET['mode']=='del_other')
					{
			             $id= $_GET['id'];
			            
						 $sql= " SELECT ass_change_own_other  FROM ass_change_other WHERE ass_change_own_other='$id' ";
							...
							{
								//exit();
							$sql = "DELETE FROM   ass_other    WHERE ass_other_id ='$id' " ;
							$result = execute_sql($database_name, $sql, $link);
								?>
							<script>alert('刪除成功！');window.location.href = 'view_other.php';</script>
								<?php
...
?>	
			...
			<table cellpadding="0" cellspacing="0" class="bar">
				<tr>
				<td width="200"><a class="add" href="insert_other.php" targer="_self" >新增資產</a></td>
				<td align="right">
	...
	?>
					...
					<!----目前寫死--->
					<select id="list" name="ass_other_label" onchange="this.form.submit();">
							...
							if(empty($B111))
							{
								...
								echo '<option value="" selected disabled="disabled">請選擇設備來源</option>' ;
								?>
								<option value="2" <?php if($_POST['ass_other_label']==2){echo 'selected'; }?>>2期</option>
								<option value="3" <?php if($_POST['ass_other_label']==3){echo 'selected'; }?>>3期</option>
								<?php
							}
...
*/
	...
	if($A>0)
	{
	//$sql4 = "SELECT * FROM `ass_other` where ass_other_label ='$A' ";
	//echo '未選擇部落';
	if($Doube_label_tribe>0 )
		{
		$sql4 = "SELECT * FROM `ass_other` where ass_other_tribe='$Doube_label_tribe' ";	
		}
	}else{
		if($city>0 ){
		$sql4 = "SELECT * FROM `ass_other` WHERE `ass_other_city`='$city'  ";

		if($town>0 )
		{
		$sql4 = "SELECT * FROM `ass_other`   where  ass_other_city='$city' and ass_other_twon='$town' ";

		if($tribe>0 )
		{
		$sql4 = "SELECT * FROM `ass_other`   where  ass_other_city='$city' and ass_other_twon='$town'  and ass_other_tribe='$tribe' ";

		if($assets_address>0 )
		{

		$sql4 = "SELECT * FROM `ass_other`   where  ass_other_city='$city' and  ass_other_twon='$town'  and ass_other_tribe='$tribe' and ass_other_address='$assets_address' ";
		if($ass_other_label > 0)
		{
		$sql4 = "SELECT * FROM `ass_other`   where  ass_other_city='$city' and  ass_other_twon='$town'  and ass_other_tribe='$tribe' and ass_other_address='$assets_address' and ass_other_label= '$ass_other_label'";
...
//echo  $sql4;
					...
					{
						$ass_other_id =$row4['ass_other_id'];
						$ass_other_city =$row4['ass_other_city'];
						$ass_other_twon =$row4['ass_other_twon'];
						$ass_other_tribe =$row4['ass_other_tribe'];
						$ass_other_address =$row4['ass_other_address'];
						$ass_other_name =$row4['ass_other_name'];
						
						////
						$ass_other_sn =$row4['ass_other_sn'];
						$ass_other_mac =$row4['ass_other_mac'];
						$ass_other_pn =$row4['ass_other_pn'];
				...
				<tr>
					...
					<td>
						...
						$sql0 = "SELECT *  FROM  city_array	  where  	id=' $ass_other_city'  ";
					...
					<td>
						<?php
							...
							$sql0 = "SELECT *  FROM  city_township  where  	township_id=' $ass_other_twon'  ";
					$result0= execute_sql($database_name, $sql0, $link);
					while ($row0 = mysql_fetch_assoc($result0))
					{
					...
					<td>
						<?php
							$sql0 = "SELECT *  FROM  tribe where  	tribe_id=' $ass_other_tribe'  ";
					$result0= execute_sql($database_name, $sql0, $link);
					while ($row0 = mysql_fetch_assoc($result0))
					{
					...
					<td>
						<?php
							$sql0 = "SELECT *  FROM  assets_address where  	ass_address_id=' $ass_other_address'  ";
					$result0= execute_sql($database_name, $sql0, $link);
					while ($row0 = mysql_fetch_assoc($result0))
					{
					...
					<td><?=	$ass_other_name;?></td>
					<td><a href="fix_other.php?LONG_TXT=<?=$LONG_TXT ;?>&id=<?=$ass_other_id;?>" target="blank"><img src="../images/icon_edit.png" class="adm_icon" align="absmiddle"></a></td>
					<td><a href="change_other.php?LONG_TXT=<?=$LONG_TXT ;?>&id=<?=$ass_other_id;?>" target="blank"><img src="../images/icon_change.png" class="adm_icon" align="absmiddle"></a></td>
					
					<td>
					<a href="javascript:if(confirm('確定要刪除嗎?'))location='?mode=del_other&id=<?=$ass_other_id;?>'"><img src="../images/icon_del.png" class="adm_icon" align="absmiddle"></a>
					</td>
					<td>
					<?php
					
					//$ass_grouter_id 
					$sql_number = "SELECT *  FROM  ass_change_other where  	ass_change_own_other=' $ass_other_id'  ";
					...
					<td  style="display: none;"><?=	$ass_other_sn;?></td>
					<td  style="display: none;"><?=	$ass_other_mac;?></td>
					<td  style="display: none;"><?=	$ass_other_pn;?></td>
				</tr>
