...
<!-------------------------------------- MAIN -->
	<div id="main">
		    ...
		    if($_GET['mode']=='del_grouter')
					{
						/*
			             $id= $_GET['id'];
							$sql = "DELETE FROM   ass_pdu    WHERE ass_pdu_id ='$id' " ;
							$result = execute_sql($database_name, $sql, $link);
								?>
							<script>alert('刪除成功！');window.location.href = 'view_pdu.php';</script>
								<?php
								* */
								$id= $_GET['id'];
			            
						 $sql= " SELECT ass_change_own_PDU  FROM ass_change_PDU WHERE ass_change_own_PDU='$id' ";
							$result= execute_sql($database_name, $sql, $link);
							//取得記錄數
		...
		<div class="tab_container">
			...
			<table cellpadding="0" cellspacing="0" class="bar">
				<tr>
				<td width="200"><a class="add" href="insert_pdu.php" targer="_self" >新增資產</a></td>
				<td align="right">
					<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" class="period">
					位置
	...
	?>
					...
					<!----目前寫死--->
					<select id="list" name="ass_pdu_label" onchange="this.form.submit();">
							...
							if(empty($B111))
							{
								...
								echo '<option value="" selected disabled="disabled">請選擇設備來源</option>' ;
								?>
								<option value="2" <?php if($_POST['ass_pdu_label']==2){echo 'selected'; }?>>2期</option>
								<option value="3" <?php if($_POST['ass_pdu_label']==3){echo 'selected'; }?>>3期</option>
								<?php
							}
...
if($A>0)
	{
	//$sql4 = "SELECT * FROM `ass_pdu` where ass_pdu_label ='$A' ";
	//echo '未選擇部落';
	if($Doube_label_tribe>0 )
		{
		$sql4 = "SELECT * FROM `ass_pdu` where  ass_pdu_tribe='$Doube_label_tribe' ";	
		}
	}else{
			if($city>0 ){
                      $sql4 = "SELECT * FROM `ass_pdu` WHERE `ass_pdu_city`='$city'  ";
			
							if($town>0 )
						{
								   $sql4 = "SELECT * FROM `ass_pdu`   where  ass_pdu_city='$city' and ass_pdu_twon='$town' ";
								
															if($tribe>0 )
										{
																   $sql4 = "SELECT * FROM `ass_pdu`   where  ass_pdu_city='$city' and ass_pdu_twon='$town'  and ass_pdu_tribe='$tribe' ";
															
												if($assets_address>0 )
															{
																	
													  $sql4 = "SELECT * FROM `ass_pdu`   where  ass_pdu_city='$city' and  ass_pdu_twon='$town'  and ass_pdu_tribe='$tribe' and ass_pdu_address='$assets_address' ";
															if($ass_pdu_label >0)
															{
																
	 $sql4 = "SELECT * FROM `ass_pdu`   where  ass_pdu_city='$city' and  ass_pdu_twon='$town'  and ass_pdu_tribe='$tribe' and ass_pdu_address='$assets_address' and ass_pdu_label='$ass_pdu_label' ";
...
//echo  $sql4;
					...
					{
						$ass_pdu_id =$row4['ass_pdu_id'];
						$ass_pdu_city =$row4['ass_pdu_city'];
						$ass_pdu_twon =$row4['ass_pdu_twon'];
						$ass_pdu_tribe =$row4['ass_pdu_tribe'];
						$ass_pdu_address =$row4['ass_pdu_address'];
						$ass_pdu_name =$row4['ass_pdu_name'];
						$IP =$row4['ass_pdu_ip'];
						
						
						////
						$ass_pdu_sn =$row4['ass_pdu_sn'];
						$ass_pdu_mac =$row4['ass_pdu_mac'];
						$ass_pdu_pn =$row4['ass_pdu_pn'];
						?>
				<tr>
...
<td><a href="change_pdu.php?LONG_TXT=<?=$LONG_TXT ;?>&LONG_TXT2=<?=$LONG_TXT2 ;?>&id=<?=$ass_pdu_id;?>" target="blank"><img src="../images/icon_change.png" class="adm_icon" align="absmiddle"></a></td>
					...
					<td  style="display: none;"><?=	$ass_pdu_sn;?></td>
					<td  style="display: none;"><?=	$ass_pdu_mac;?></td>
					<td  style="display: none;"><?=	$ass_pdu_pn;?></td>
				</tr>
