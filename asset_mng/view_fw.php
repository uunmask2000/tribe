...
?>
		...
		<div class="tab_container">
			...
			<table cellpadding="0" cellspacing="0" class="bar">
				<tr>
				<td width="200"><a class="add" href="insert_fw.php" targer="_self" >新增資產</a></td>
				<td align="right">
...
<?php         //  echo       $string =  $B.'-'.$B1.'-'.$B11.'-'.$B111.'-'.$_POST['view_num'];        ?>
			<table id="show_date" class="asset">
				...
				if($A>0)
				{
					//$sql4 = "SELECT * FROM `ass_grouter` where ass_grouter_label ='$A' ";
					//echo '未選擇部落';
					if($Doube_label_tribe>0 )
						{
						$sql4 = "SELECT * FROM `ass_grouter` where ass_grouter_tribe='$Doube_label_tribe' ";	
						}
				}else{
						if($city>0 )
						{
	...
	$result4= execute_sql($database_name, $sql4, $link);
				...
				<tr>
					...
					<td><?=	$ass_name;?></td>
					<td><a href="fix_fw.php?LONG_TXT=<?=$LONG_TXT ;?>&id=<?=$ass_grouter_id;?>" target="blank"><img src="../images/icon_edit.png" class="adm_icon" align="absmiddle"></a></td>
					<td><a href="change_fw.php?LONG_TXT=<?=$LONG_TXT ;?>&LONG_TXT2=<?=$LONG_TXT2 ;?>&id=<?=$ass_grouter_id;?>" target="blank"><img src="../images/icon_change.png" class="adm_icon" align="absmiddle"></a></td>
					
					<td>
					<a href="javascript:if(confirm('確定要刪除嗎?'))location='?mode=del_grouter&id=<?=$ass_grouter_id;?>'"><img src="../images/icon_del.png" class="adm_icon" align="absmiddle"></a>
					</td>
					<td>
					<?php
					...
					?>
						...
						<td  style="display: none;"><?=	$ass_sn;?></td>
						<td  style="display: none;"><?=	$ass_mac;?></td>
						<td  style="display: none;"><?=	$ass_pn;?></td>
				</tr>
