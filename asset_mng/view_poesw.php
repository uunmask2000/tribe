...
<!-------------------------------------- MAIN -->
	<div id="main">
		    ...
		    if($_GET['mode']=='del_grouter')
					{
						/*
			             $id= $_GET['id'];
							$sql = "DELETE FROM   ass_poesw    WHERE ass_poesw_id ='$id' " ;
							$result = execute_sql($database_name, $sql, $link);
								?>
							<script>alert('刪除成功！');window.location.href = 'view_poesw.php';</script>
								<?php
								*/
								$id= $_GET['id'];
			            
						 $sql= " SELECT ass_change_own_poe_sw  FROM ass_change_poe_sw WHERE ass_change_own_poe_sw='$id' ";
							$result= execute_sql($database_name, $sql, $link);
							//取得記錄數
		...
		<div class="tab_container">
			...
			<table cellpadding="0" cellspacing="0" class="bar">
				<tr>
				<td width="200"><a class="add" href="insert_poesw.php" targer="_self" >新增資產</a></td>
				<td align="right">				
	...
	?>
					...
					<!----目前寫死--->
					<select id="list" name="ass_poesw_label" onchange="this.form.submit();">
							...
							if(empty($B111))
							{
								...
								echo '<option value="" selected disabled="disabled">請選擇設備來源</option>' ;
								?>
								<option value="2" <?php if($_POST['ass_poesw_label']==2){echo 'selected'; }?>>2期</option>
								<option value="3" <?php if($_POST['ass_poesw_label']==3){echo 'selected'; }?>>3期</option>
								<?php
							}
...
if($A>0)
	{
	///$sql4 = "SELECT * FROM `ass_poesw` where ass_poesw_label ='$A' ";
	//echo '未選擇部落';
	if($Doube_label_tribe>0 )
		{
		$sql4 = "SELECT * FROM `ass_poesw` where ass_poesw_tribe='$Doube_label_tribe' ";	
		}
	}else{
			if($city>0 ){
                      $sql4 = "SELECT * FROM `ass_poesw` WHERE `ass_poesw_city`='$city'  ";
			
							if($town>0 )
						{
								   $sql4 = "SELECT * FROM `ass_poesw`   where  ass_poesw_city='$city' and ass_poesw_twon='$town' ";
								
															if($tribe>0 )
										{
																   $sql4 = "SELECT * FROM `ass_poesw`   where  ass_poesw_city='$city' and ass_poesw_twon='$town'  and ass_poesw_tribe='$tribe' ";
															
												if($assets_address>0 )
															{
																	
													  $sql4 = "SELECT * FROM `ass_poesw`   where  ass_poesw_city='$city' and  ass_poesw_twon='$town'  and ass_poesw_tribe='$tribe' and ass_poesw_address='$assets_address' ";
															
															if($ass_poesw_label > 0)
															{
														  $sql4 = "SELECT * FROM `ass_poesw`   where  ass_poesw_city='$city' and  ass_poesw_twon='$town'  and ass_poesw_tribe='$tribe' and ass_poesw_address='$assets_address' and ass_poesw_label ='$ass_poesw_label' ";
...
//echo  $sql4;
					...
					{
						$ass_poesw_id =$row4['ass_poesw_id'];
						$ass_poesw_city =$row4['ass_poesw_city'];
						$ass_poesw_twon =$row4['ass_poesw_twon'];
						$ass_poesw_tribe =$row4['ass_poesw_tribe'];
						$ass_poesw_address =$row4['ass_poesw_address'];
						$ass_poesw_name =$row4['ass_poesw_name'];
						$IP =$row4['ass_poesw_ip'];
						////
						$ass_poesw_sn =$row4['ass_poesw_sn'];
						$ass_poesw_mac =$row4['ass_poesw_mac'];
						$ass_poesw_pn =$row4['ass_poesw_pn'];
				...
				<tr>
					...
					<td><a href="fix_poesw.php?LONG_TXT=<?=$LONG_TXT ;?>&id=<?=$ass_poesw_id;?>" target="blank"><img src="../images/icon_edit.png" class="adm_icon" align="absmiddle"></a></td>
					<td><a href="change_poesw.php?LONG_TXT=<?=$LONG_TXT ;?>&LONG_TXT2=<?=$LONG_TXT2 ;?>&id=<?=$ass_poesw_id;?>" target="blank"><img src="../images/icon_change.png" class="adm_icon" align="absmiddle"></a></td>
					
					<td>
					<a href="javascript:if(confirm('確定要刪除嗎?'))location='?mode=del_grouter&id=<?=$ass_poesw_id;?>'"><img src="../images/icon_del.png" class="adm_icon" align="absmiddle"></a>
					</td>
						<td>
					<?php
					...
					<td  style="display: none;"><?=	$ass_poesw_sn;?></td>
					<td  style="display: none;"><?=	$ass_poesw_mac;?></td>
					<td  style="display: none;"><?=	$ass_poesw_pn;?></td>
				</tr>
