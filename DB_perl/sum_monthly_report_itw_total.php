...
//$month_row = '2016-09';
$month_row =date("Y-m", strtotime('0 month'));
...
while ($row_tribe = mysql_fetch_array($result_tribe))
{
			...
			$sql_ap1 = "SELECT * FROM `ass_ap` WHERE `ass_ap_tribe`='$tribe_id' ";
			$result_ap1 = execute_sql($database_name, $sql_ap1, $link);				
			$device_number  = mysql_num_rows($result_ap1); //設備數量
			...
			//$device_number  = mysql_num_rows($result_ap); //設備數量
...
$ass_ap_ip = "'".$ass_ap_ip."'";
//SELECT * FROM `alert_ap_date_filter` WHERE `alert_ap_date_ap_ip` IN ('172.21.23.101','172.21.23.111','172.21.23.112','172.21.23.121')
//$sql_00 = "SELECT * FROM `alert_ap_date_filter` WHERE alert_ap_date_ap_ip` IN ($ass_ap_ip)  and alert_ap_date_time_dead like '%$month_row%' ";
$sql_00 = " SELECT * FROM `alert_ap_date_filter` WHERE `alert_ap_date_ap_ip` IN ($ass_ap_ip)   and alert_ap_date_time_dead like '%$month_row%'   ";
...
//echo '<br>' ;
						...
						$sq3l = " SELECT * FROM monthly_report_itw_total where setting='$stting_setting' and Time_interval='$month_row'  ";
						$result31 =  execute_sql2($database_name2, $sq3l, $link2);
						$number31 = mysql_num_rows($result31);	
						if($number31==0)
						{
							$sql2 = "INSERT INTO `monthly_report_itw_total`(`Period`, `County`, `area`, `tribe`, `Use_of_people`, `Number_of_users`, `Total_usage_time`, `Upload_traffic`, `Download_traffic`, `setting`, `Time_interval`,county_sid,area_sid,tribe_sid,filter_number,device_number) VALUES ('$tribe_label','$city_name','$township_name','$tribe_name','$number1','$number2','$acctsessiontime','$acctinputoctets','$acctoutputoctets','$stting_setting','$month_row','$city_id','$township_id','$tribe_id','$num_rows','$device_number')";
...
{							
$id = $row['monthly_report_itw_total_id'];

//UPDATE
$sql3 = "UPDATE `monthly_report_itw_total` SET device_number='$device_number', Use_of_people ='$number1',Number_of_users ='$number2',Total_usage_time ='$acctsessiontime',Upload_traffic ='$acctinputoctets',Download_traffic ='$acctoutputoctets',filter_number ='$num_rows' where monthly_report_itw_total_id ='$id '  ";
