...
while ($row_tribe = mysql_fetch_array($result_tribe))
{
			...
			{	
					...
					////
					$sql1="SELECT radacctid FROM radacct where  nasipaddress IN ('$ass_ap_ip')  and  realm='0'  and acctstarttime like '%$month_row%'";
					$result1 = execute_sql2($database_name2, $sql1, $link2);
					$number1 = mysql_num_rows($result1); //紀錄使用人次
				//echo $number1;
				//echo '<br>';
					///
					$sql2="SELECT callingstationid   FROM radacct where  nasipaddress IN ('$ass_ap_ip')   and  realm='0'  and acctstarttime like '%$month_row%'  GROUP BY callingstationid ";
					$result2 = execute_sql2($database_name2, $sql2, $link2);
					$number2 = mysql_num_rows($result2); //紀錄使用人數
				//echo $number2;
				//echo '<br>';
						//
					$sql_sum = "SELECT SUM(acctsessiontime),SUM(acctinputoctets),SUM(acctoutputoctets) FROM radacct  where nasipaddress IN ('$ass_ap_ip') and  realm='0'  and acctstarttime like '%$month_row%' ";
	...
	$num_rows= mysql_num_rows($result_00); 
						...
						$sq3l = " SELECT * FROM monthly_report_itr where setting='$stting_setting' and Time_interval='$month_row'  ";
...
{							
$id = $row['monthly_report_itr_id'];

//UPDATE
$sql3 = "UPDATE `monthly_report_itr` SET Use_of_people ='$number1',Number_of_users ='$number2',Total_usage_time ='$acctsessiontime',Upload_traffic ='$acctinputoctets',Download_traffic ='$acctoutputoctets',filter_number ='$num_rows' where monthly_report_itr_id ='$id '  ";
