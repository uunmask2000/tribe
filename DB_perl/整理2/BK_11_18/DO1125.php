...
// $month_row =date("Y-m", strtotime('-1 month'));
 $month_row1 =date("Y-m", strtotime('0 month'));
	...
	where serviceid=2 and iflostservice like '$month_row' ORDER BY iflostservice desc";
	//where serviceid=2 and iflostservice >'$month_row' ORDER BY iflostservice desc*/
	...
	while ($row_outages = pg_fetch_row($result_outages) )
	{
		...
		/////
		   ...
		   ///
			...
			$sql = "UPDATE alert_ap_date SET alert_ap_date_time_dead='$YMD_string',alert_ap_date_time_ok='$YMD_string_2',alert_ap_date_setting='$alert_ap_date_setting' WHERE alert_ap_date_outageid='$outageid_pg' and alert_ap_date_svclosteventid ='$svclosteventid_pg'";
			execute_sql($database_name, $sql, $link);
			//echo $sql;
			//echo '<br> ';
			}
