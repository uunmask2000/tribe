...
<?php
$sql_Mm = "SELECT MAX(radacctid),MIN(radacctid) FROM radacct  where  realm<>'itw'  and acctstarttime like '%2016-10%' ";
$result_Mm = execute_sql2($database_name2, $sql_Mm, $link2);
while ($row_Mm = mysql_fetch_array($result_Mm))
{	
...
while ($row_t = mysql_fetch_array($result_t))
{
	for($tt=0 ; $tt <32 ;$tt++)
	{
				...
				$time_zonw_ymd = "2016-10-01";
				$time_zonw_ymd = strtotime($time_zonw_ymd);
				$time_zonw_ymd = strtotime("+ $tt day", $time_zonw_ymd);
				$time_zonw_ymd = date('Y-m-d', $time_zonw_ymd);
				...
				$sql_sum = "SELECT SUM(acctinputoctets),SUM(acctoutputoctets) FROM radacct  where nasipaddress IN ('$ass_ap_ip') and  realm<>'itw'  and acctstarttime like '%$time_string%' and radacctid >='$MIX_ID' and radacctid <='$MAX_ID'  "; 
				...
				$sql2 = "INSERT INTO `sum_day_hr_itr`(`Period`, `County`, `area`, `tribe`, `time_zone_h`, `Upload_traffic`, `Download_traffic`, `county_sid`, `area_sid`, `tribe_sid`, `setting_id`) VALUES ('$Period', '$city_name', '$township_name', '$tribe_name', '$time_string', '$acctinputoctets', '$acctoutputoctets', '$city_id', '$township_id', '$tribe_id', '$setting_id')";
				execute_sql($database_name2, $sql2, $link2);
				
				/*
				$sq3l = " SELECT * FROM sum_day_hr_itr where setting_id='$setting_id' and time_zone_h='$time_string'  ";
...
echo   "Time ".($end_time-$count_time)."s";
