	   ...
	   date_default_timezone_set("Asia/Taipei");
	  	//require_once("dbtools.inc.php");
			...
			$link = create_connection();
				...
				$sql_PDU="SELECT * FROM PDU_check					";
				$result_PDU= execute_sql($database_name, $sql_PDU, $link);
				while ($row_PDU= mysql_fetch_assoc($result_PDU)  )
				{
						$PDU_check_tribe  = $row_PDU['PDU_check_tribe'];
						//echo '這是第一行<br>';
						$PDU_check_IP   = $row_PDU['PDU_check_IP'];
						//echo '這是第一行<br>';
						$PDU_check_state     = $row_PDU['PDU_check_state'];
						$PDU_check_written     = $row_PDU['PDU_check_written'];
					?>
					<tr>
						<td><?=$PDU_check_tribe ;?></td>
						<td><?=$PDU_check_IP ;?></td>
						<td><?=$PDU_check_state ;?></td>
						<td><?=$PDU_check_written ;?></td>
					</tr>
					<?php
