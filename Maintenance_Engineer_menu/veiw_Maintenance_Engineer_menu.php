...
<!-------------------------------------- MAIN -->
	...
	<?php include("../include/nav.php"); ?>
		...
		<tr><td width="200"><a class="add" href="#" onclick="insert()" >新增工程師</a></td></tr>
		</table>
		...
		<table id="table1" class="asset">
			...
			$link = create_connection();
				$j = 1 ;
				$sql_view="SELECT * FROM  Maintenance_Engineer_menu	 ORDER by Maintenance_Engineer_menu_id asc";
				$result_view= execute_sql($database_name, $sql_view, $link);
				while ($row_view= mysql_fetch_assoc($result_view)  )
				{
						$Maintenance_Engineer_menu_id=$row_view['Maintenance_Engineer_menu_id'];
						$alert_contacts_email=$row_view['alert_contacts_email'];
						$alert_contacts_name=$row_view['alert_contacts_name'];
						?>
						<tr>
							<td><?=$j;?></td>
							<td><?=$row_view['Maintenance_Engineer_menu_name'];?></td>							
							<td>
							<a href="Identification.php?mode=A2&key=<?=$Maintenance_Engineer_menu_id;?>"><img src="../images/icon_edit.png" class="adm_icon" align="absmiddle"></a>
							</td>
							<td>
							<a href="javascript:if(confirm('確定要刪除嗎??'))location='Identification.php?mode=A3&key=<?=$Maintenance_Engineer_menu_id;?>'" ><img src="../images/icon_del.png"  class="adm_icon" align="absmiddle"></a>
							</td>
						</tr>
						<?php
						$j++ ;
