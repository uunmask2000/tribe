...
if($day > '2017-03-01')
{
//$sql_sum="SELECT * FROM `sum_day_hr_itw` WHERE    Period ='$Period'  and `time_zone_h` like '%$day%' ORDER BY tribe asc,time_zone_h asc ";

							$sql_sum="SELECT *,sum(Upload_traffic),sum(Download_traffic) 
									  FROM sum_day_hr_itw 
									  where Period ='$Period'  and `time_zone_h` like '%$day%'
									  GROUP BY tribe 
							...
							for($ii=0;$ii < $jjj ;$ii++)
										{
											?>
													<tr>
														...
														<td><?=$array2[$ii][3] ; ?></td>
														<td><?php 
															$SUM_2  = (($array2[$ii][11]+$array1[$ii][11]) +($array2[$ii][12]+$array1[$ii][12]))/(1024*1000) ;
															//echo $SUM_2 ;
															echo ceil($SUM_2) ;
																   ?></td>
														<td><?php
														$Upload_traffic=($array2[$ii][11]+$array1[$ii][11])/(1024*1000) ; 
														echo ceil($Upload_traffic) ;
														?></td>
														<td><?php
														$Download_traffic =($array2[$ii][12]+$array1[$ii][12])/(1024*1000) ;
...
$(document).ready(function(){ 
  var opt={ "oLanguage":{"sProcessing":"處理中...",
			 ...
			 {
				 ?>
				{ extend: 'excelHtml5', text: '匯出服務效益日總報表' ,title: '<?= date("Y-m-d");?>服務效益日總報表' }, 
				 <?php
			 }
			 ?>
