		...
		if($msger =='1')
		{
			...
			<table id="show_date" class="display" cellspacing="0" width="100%">
				...
				//$year	 $month	
							...
							?>	</td>
							<td align="center">
							<a href="report_web.php?mode=case_E&ip=<?=$Device_IP;?> &realm=愛部落&tribe_name=<?=$tribe;?>&label=<?=$Period;?>&apname=<?=$aroused_general_interest;?>" target="_blank">
							...
							$sql3="SELECT * FROM monthly_report_itw WHERE tribe_sid='$tribe_sid' and Time_interval='$Time_interval'";
							...
							$SUM_2  = number_format(($row1['Upload_traffic']/(1024*1000))+($row1['Download_traffic']/(1024*1000)) );
							$Total_usage_time=  number_format($Total_usage_time);
							$Upload_traffic=  number_format($Upload_traffic);
							$Download_traffic= number_format($Download_traffic);
							?>
							...
							?>	</td>
							<td align="center">
							<a href="report_web.php?mode=case_E&ip=<?=$Device_IP;?> &realm=愛台灣&tribe_name=<?=$tribe;?>&label=<?=$Period;?>&apname=<?=$aroused_general_interest;?>" target="_blank">
							<img src="https://cdn0.iconfinder.com/data/icons/woocons1/Chart.png"></a>
							</td>
							</tr>
							<?php
