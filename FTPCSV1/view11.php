...
//exit();
        ...
        }else
	{
		...
		$sql1 = "SELECT * FROM  new_Client_Associations  WHERE   timestamp='$data[0]' and zone='$CCCC[1]'        " ;
	...
	//echo $sql1;
		...
		if($EEEE[1]=='Ssid:iTaiwan')
		{
 		...
 		$sql ="INSERT INTO new_Client_Associations
		...
		}else
		{
 		...
 		$sql ="INSERT INTO new_Client_Associations
		(data_interval, time_zone, zone, radio, timestamp ,ssid_itribe, itribe_fail_ass, ssid_itaiwan, itaiwan_fail_ass)
		 VALUES 
		 ('$AAAA[0]','$BBBB[0]','$CCCC[1]','$DDDD[1]','$data[0]','$EEEE[1]','$data[1]','$EEEE[2]','$data[2]')";
		$result = execute_sql($database_name, $sql, $link);
...
$sql1 = "SELECT * FROM  new_Client_Associations   WHERE   timestamp='$data[0]' and zone='$CCCC[$z1]'        " ;
		$result1 = execute_sql($database_name, $sql1, $link);
		$total_records = mysql_num_rows($result1);
	//echo $sql1;
		if($total_records==0)
		...
		(data_interval, time_zone, zone, radio, timestamp, ssid_itaiwan, itaiwan_fail_ass, ssid_itribe, itribe_fail_ass)
		 VALUES 
		 ('$AAAA[0]','$BBBB[0]','$CCCC[$z1]','$DDDD[1]','$data[0]','$EEEE[$z1]','$data[1]','$EEEE[$z2]','$data[2]')";
		$result = execute_sql($database_name, $sql, $link);
