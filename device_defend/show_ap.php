...
//echo $_GET['ip'];
	...
	echo '<H1>';
	$sql_router = "SELECT *  FROM  ass_ap where ass_ap_ip='$ip '  ";
	$result_router = execute_sql($database_name, $sql_router, $link);
	while ($row_router = mysql_fetch_assoc($result_router))
	{
		$ass_ap_city =  $row_router['ass_ap_city'];
		$ass_ap_twon =  $row_router['ass_ap_twon'];
		$ass_ap_tribe =  $row_router['ass_ap_tribe'];
		$ass_ap_ip =  $row_router['ass_ap_ip'];
		$ass_ap_name =  $row_router['ass_ap_name'];
		$ass_ap_id =  $row_router['ass_ap_id'];
		//echo $ass_ap_name ;
			echo '<a href="http://'.$ass_ap_ip.'">'.$ass_ap_name.'</a>';
	}
	echo '</H1>';
	echo '<H2>';
	...
	echo '</H2>';
		?>
			...
			*/
				$sql_change = "SELECT *  FROM  ass_change_ap where ass_change_own_ap='$ass_ap_id' ORDER BY ass_change_time_ap asc ";
				$result_change = execute_sql($database_name, $sql_change, $link);
				while ($row_change = mysql_fetch_assoc($result_change))
				{
						?>
						<tr>
						<td><?=$row_change['ass_change_time_ap'] ;?></td>
						<td><?=$row_change['ass_change_name_ap'] ;?></td>
						<td><?=$row_change['ass_change_sn_ap'] ;?></td>
						<td><?=$row_change['ass_change_mac_ap'] ;?></td>
						<td><?=$row_change['ass_change_pn_ap'] ;?></td>
						<td><?=$row_change['ass_change_note_ap'] ;?></td>
						<td><?=$row_change['ass_change_label_ap'] ;?></td>
						</tr>
						<?php
	...
	//$conn = pg_connect("host=127.0.0.1 port=5432 dbname=opennms user=opennms password=0932969495");
						...
						where A.ipaddr ='$ass_ap_ip'  and B.serviceid = 2
						order by c.outageid desc";
						...
						?></td>
						<td><?php
						//echo $row_ipinterface['ifregainedservice']; 
